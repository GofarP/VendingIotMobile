...
import { useGetPermissionCategory } from "../hooks/PermissionCategory/usePermissionCategoryQuery";
import { usePermissionCategoryActions } from "../hooks/PermissionCategory/usePermissionCategoryAction";
import { PermissionCategory } from "../types/permissionCategory";
import { useSnackbar } from "../components/SnackbarContext";
import { Pencil, Trash2 } from 'lucide-react-native';

export default function PermissionCategoryScreen() {
    const { showSnackbar } = useSnackbar();

    const [search, setSearch] = useState('');
    const [modalVisible, setModalVisible] = useState(false);

    const { data, isLoading, refetch, isRefetching } = useGetPermissionCategory(1, search);
    ...
    const {
        ...
        isSubmitting,
        selectedId,
    } = usePermissionCategoryActions();

    const handleOpenModal = (item?: PermissionCategory) => {
        openModal(item);
    ...
    return (
        <PageWrapper title="Permission Category">
            <DataList
                data={data?.data || []}
                ...
                renderItem={({ item }: { item: PermissionCategory }) => (
                    <TouchableOpacity
                        activeOpacity={0.7}
            ...
            <AppModal
                ...
                title={selectedId ? "Update Permission Category" : "Create Permission Category"}
            ...
            >
                ...
                <Input
                    label="Description"
                    placeholder="Explain about this permission category"
